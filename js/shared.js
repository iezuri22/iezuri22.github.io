...
function navigateTo(view) {
  ...
  if (view === 'my-plates' || view === 'cooking-journal') { sessionStorage.setItem('savedPageTab', 'journal'); view = 'saved'; }
  if (view === 'plates-page') { sessionStorage.setItem('savedPageTab', 'plates'); view = 'saved'; }
...
function renderNav() {
  ...
  const pageTitles = { 'recipes': 'Recipes', 'my-meals': 'Home', 'food-log-detail': 'Meal Detail', 'recipe-edit': 'Edit Recipe', 'recipe-view': 'Recipe', 'freestyle-edit': 'Freestyle', 'batch-edit': 'Build a Plate', 'batch-view': 'Plate', 'grocery-list': 'Grocery', 'grocery-select-meals': 'Select Meals', 'grocery-ingredients': 'Ingredients', 'inventory': 'Pantry', 'ingredients': 'Ingredients', 'ingredient-detail': 'Ingredient', 'kitchen': 'My Kitchen', 'kitchen-detail': 'Ingredient', 'saved': 'Saved' };
...
function renderBottomNav() {
  ...
  const navPaddingBottom = isStandalone ? '34px' : 'env(safe-area-inset-bottom)';

  return `<nav class="bottom-nav" style="position: fixed; bottom: 0; left: 0; right: 0; background: ${CONFIG.surface_color}; box-shadow: 0 -1px 8px rgba(0,0,0,0.3); z-index: 40; padding-bottom: ${navPaddingBottom};"><div style="display: flex; justify-content: space-around; align-items: center; padding: 6px 0 4px;">${navItems.map(item => {
