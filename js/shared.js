...
function renderNav() {
  ...
  const backButton = isMainNavPage ? '' : `<button onclick="if(window.history.length>1){window.history.back();}else{navigateTo('${backTarget}');}" style="color: ${CONFIG.text_color}; background: none; border: none; cursor: pointer; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; padding: 0;"><svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"/></svg></button>`;
